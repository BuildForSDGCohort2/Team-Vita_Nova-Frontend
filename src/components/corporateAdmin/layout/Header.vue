<template>
  <div>
    <v-app-bar app color="white" light elevate-on-scroll clipped-left>
      <v-btn class="drawer" @click.stop="drawer = !drawer" color="white" elevation="0">
    ...
    <v-navigation-drawer v-model="drawer" app color="#FAFAFA" clipped hide-overlay>
      <v-list nav dense>
        <v-list-item link to="/dashboard" class="dashboard-logout" color="red">
          <v-list-item-icon>
...
export default {
  ...
  computed: {
    user() {
      return this.$store.state.onboarding.user.first_name || userData.first_name;
    },
    profilePicture() {
